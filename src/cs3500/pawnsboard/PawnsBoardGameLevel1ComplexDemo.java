...
public class PawnsBoardGameLevel1ComplexDemo {
  ...
  public static void main(String[] args) {
    try {
      ...
      // Add RED pawns to position (1,3) for final demonstration
      try {
        // Actually add the pawns (this was missing)
        for (int i = 0; i < 2; i++) {
          model.getCell(1, 3).addPawn(PlayerColors.RED);
        }
        System.out.println("Added RED pawns to position (1,3) for final demonstration");
        System.out.println(view.renderGameState("After adding pawns to (1,3)"));
        System.out.println();
      } catch (Exception e) {
        System.err.println("Error adding pawns: " + e.getMessage());
      }
