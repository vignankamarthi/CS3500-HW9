...
/**
 * An augmented panel that displays the Pawns Board game board with value modifiers.
 * This panel extends the functionality of GameBoardPanel to support the display
 * of value modifiers and different influence types.
 *
 ...
 */
public class AugmentedGameBoardPanel implements BoardPanel {
   ...
   */
  public AugmentedGameBoardPanel(ReadOnlyPawnsBoard<?, ?> model, ColorSchemeManager
          colorSchemeManager) {
    if (model == null) {
  ...
  private void drawCell(Graphics2D g2d, int row, int col, int xOffset, int yOffset) {
    ...
    try {
      ...
      // Get value modifier from augmented model
      int valueModifier = augmentedModel.getCellValueModifier(row, col);

      // Use AugmentedDrawingUtils to draw cell background with content, owner, and value modifier
      AugmentedDrawingUtils.drawCellWithValueModifier(g2d, bounds,
      ...
      // Just draw a basic cell without content
      DrawingUtils.drawCellBackground(g2d, bounds,
              row == highlightedRow && col == highlightedCol,
              scheme, CellContent.EMPTY, null);
    }
  }
  ...
  private void drawPawns(Graphics2D g2d, int row, int col, int x, int y) {
    try {
      ...
      AugmentedDrawingUtils.drawPawnsWithValueModifier(g2d, bounds, count, owner,
              colorSchemeManager.getColorScheme(), valueModifier);
    } catch (IllegalArgumentException | IllegalStateException e) {
  ...
  private void drawCard(Graphics2D g2d, int row, int col, int x, int y) {
    try {
      ...
      AugmentedDrawingUtils.drawCellCardWithModifier(g2d, bounds, value, valueModifier,
              owner, colorSchemeManager.getColorScheme());
    } catch (IllegalArgumentException | IllegalStateException e) {
      // Cell may be invalid or game not started
    }
  }
  ...
  private void drawUnstartedGameMessage(Graphics2D g2d) {
    ...
    String message = "Augmented Game not started";
